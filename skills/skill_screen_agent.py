...
_SYSTEM_PROMPT = (
    ...
    "- IMPORTANT: Always call focus_window() on the target app IMMEDIATELY before keyboard_type() or keyboard_key(). This ensures text goes to the correct window.\n"
    "- For web: use open_app() with full https:// URL, then screenshot_read() after page load\n"
    "- For address bar: focus_window(browser), then keyboard_key('ctrl+l'), then keyboard_type(url), then keyboard_key('enter')\n"
...
_XML_TOOL_RE = re.compile(r'<function=(\w+)[^{<]*(\{[^<]*?\})?', re.DOTALL)
